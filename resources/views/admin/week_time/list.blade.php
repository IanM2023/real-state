                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center flex-wrap">
                        ...
                        <div class="d-flex align-items-center">
                            <a href="{{ url('admin/week/add-time') }}" class="btn btn-sm btn-success">Add Week Time</a>
                        </div>
                    </div>
                        ...
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                @forelse ($getRecord as $value)
                                    <tr>
                                        <td>{{ $value->id }}</td>
                                        <td>{{ $value->week_time }}</td>
                                        <td>{{ $value->time_format }}</td>
                                        <td>{{ date('d-m-Y', strtotime($value->created_at)) }}</td>
                                        <td class="text-center" style="width: 50px;">
                                            <a href="{{ url('admin/week_time/edit/' . $value->id) }}" class="dropdown-item align-items-center">
                                        ...
                                        <td class="text-center" style="width: 50px;">
                                            <a href="{{ url('admin/week_time/delete/' . $value->id) }}"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="100%" class="text-center py-4">No Time Record Found.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
